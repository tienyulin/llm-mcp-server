...
def test_mock_embed_golden_values():
    """mock_embed reproduces the exact golden vector shared with the processor."""
...
def test_mock_embed_deterministic_and_normalized():
    """Same input yields the same L2-normalized vector across calls."""
...
def test_query_embedder_mock_mode(monkeypatch):
    """In mock mode aembed_query matches the standalone mock_embed output."""
...
def test_query_embedder_disabled_without_config(monkeypatch):
    """No base URL and mock off means the embedder reports itself disabled."""
